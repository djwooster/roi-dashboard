...
export default function IntegrationsPage() {
  ...
  return (
    <div className="px-6 py-5 max-w-[900px]">
      ...
      {/* Connected GHL locations — auto-populated after each OAuth connect.
          Each time the user clicks Connect + picks a GHL sub-account, a row
          is upserted into ghl_locations and appears here automatically. */}
...
// Read-only list of locations connected via GHL sub-account OAuth.
// Locations appear here automatically after each OAuth flow completes —
// no manual sync needed. To add another location, the user clicks "Connect"
// on the GHL card above and picks a different sub-account from GHL's chooser.
  ...
  return (
    <div className="mb-6">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-[11px] text-[#a3a3a3]">{locations.length} active</span>
      </div>

      {/* Search — only shown when there are enough locations to warrant it */}
      {locations.length > 4 && (
      ...
      <div className="border border-[#e5e5e5] rounded-lg overflow-hidden divide-y divide-[#f5f5f5] max-h-[360px] overflow-y-auto">
        {filtered.length === 0 ? (
          ...
          filtered.map((loc) => (
            <div key={loc.location_id} className="flex items-center justify-between px-4 py-2.5 bg-white">
      ...
      <p className="text-[11px] text-[#a3a3a3] mt-2">
        To add another location, click <strong>Connect</strong> above and select a different GHL sub-account.
      </p>
    </div>
  );
}
